...
import {
    getOrderByUserAndStatusRequest,
    getOrderByUserRequest,
    getOrderFromOwncar,
    getOrderFromOwncarByStatus
} from "../../axios/lib/order/order";
...
const AccountRequestList = () => {
    ...
    const getAllOrder = () => {
        getOrderFromOwncar(user.id || "").then(res=> {
            setOrders(res.data)
        }).catch(e=> {
            if(e.response) {
    ...
    const renderSection1 = () => {
        return (
            <div className="space-y-6 sm:space-y-8">
                <div>
                    <h2 className="text-3xl font-semibold">Заявки по машинам</h2>
                </div>
                                        ...
                                        <>
                                            <div className="lg:p-10 lg:bg-neutral-50 lg:dark:bg-black/20 grid grid-cols-1 gap-6  rounded-3xl">
                                                {orders && orders.map((order) => (
                                                    <OrderCarCard data={order} ownCarRequest={true}/>
                                                ))}
                                            </div>
